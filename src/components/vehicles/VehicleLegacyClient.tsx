          ...
          <form onSubmit={submitForm} className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              {data.module.fields.map((field) => (
                <FormField key={field.key} field={field} value={form[field.key] ?? ""} refs={data.refs} onChange={(value) => setForm((old) => ({ ...old, [field.key]: value }))} />
              ))}
            </div>
...
function FormField({
  ...
  if (field.type === "textarea") {
    input = <textarea value={value} onChange={(event) => onChange(event.target.value)} className="min-h-24 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold" />;
    ...
    input = <input value={value} onChange={(event) => onChange(event.target.value)} type={field.type} className={baseClass} required={field.required} />;
  }
