...
async function getData(params: Record<string, string | string[] | undefined>) {
  const [accounts, rankRecords, performanceRecords, invoiceRecords, movements, projects, cities] = await Promise.all([
  ...
  for (const [key, rows] of accountGroups) {
    ...
    if (rows.length > 1 || drivers.size > 1) {
      ...
      alerts.set(`account:${key}`, {
        ...
        month: "-",
        drivers: rows.map((row) => row.driver ? `${row.driver.actualName || row.driver.name} (${row.driver.internalCode})` : "بدون مندوب"),
  ...
  function pushSource(input: {
    ...
    if (!input.applicationProjectId || !input.courierId) return;
    const key = `source:${input.applicationProjectId}:${input.month || "no-month"}:${input.courierId}`;
    const current = alerts.get(key) ?? {
      ...
      appName: "Keeta",
      appUserId: input.courierId,
      appUsername: input.courierName || "-",
      month: input.month || "-",
      drivers: [] as string[],
      sources: [] as string[],
      severity: "warning" as const,
      reason: "حساب ظاهر في ملفات مختلفة ويحتاج مراجعة ربط",
    };
  ...
  for (const row of rankRecords) {
    pushSource({
      ...
      driverName: row.driver ? `${row.driver.actualName || row.driver.name} (${row.driver.internalCode})` : "بدون مندوب",
      month: row.month,
    });
  }
  for (const row of performanceRecords) {
    pushSource({
      ...
      cityName: row.city?.nameAr || row.city?.nameEn,
      courierId: row.courierId,
      courierName: `${row.courierFirstName || ""} ${row.courierLastName || ""}`.trim(),
      driverId: row.driverId,
      driverName: row.driver ? `${row.driver.actualName || row.driver.name} (${row.driver.internalCode})` : "بدون مندوب",
      month: row.month,
    });
  ...
  const filteredAlerts = [...alerts.values()]
    .map((alert) => {
      ...
      return {
        ...
        drivers,
        sources,
        severity: drivers.length > 1 || drivers.includes("بدون مندوب") ? "danger" as const : "warning" as const,
        reason: drivers.length > 1 ? "نفس حساب التطبيق ظهر مع أكثر من مندوب" : alert.reason,
      };
    })
    .filter((alert) => alert.drivers.length > 1 || alert.drivers.includes("بدون مندوب") || alert.sources.length > 1)
...
export default async function AccountMovementPage({ searchParams }: PageProps) {
  ...
  return (
    <PageShell
      ...
      description="مراجعة الحسابات التي انتقلت بين مناديب أو ظهرت في ملفات متعددة داخل نفس مشروع المدينة."
      actions={
    ...
    >
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <form className="grid gap-3 lg:grid-cols-6">
          ...
          <div className="flex items-end gap-2">
            ...
            <Link href="/account-movement" className="grid h-10 place-items-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-700">
              عرض الكل
            </Link>
          </div>
        </form>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm font-bold text-slate-500">حسابات ضمن الفلتر</p>
          <strong className="mt-2 block text-3xl font-black text-slate-950">{result.accounts.length}</strong>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm font-bold text-slate-500">تنبيهات حركة</p>
          <strong className="mt-2 block text-3xl font-black text-amber-700">{result.alerts.length}</strong>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm font-bold text-slate-500">سجل حركات محفوظ</p>
          <strong className="mt-2 block text-3xl font-black text-blue-700">{result.movements.length}</strong>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm font-bold text-slate-500">المنطق المعتمد</p>
          <strong className="mt-2 block text-lg font-black text-emerald-700">ApplicationProject + City</strong>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1100px] text-right text-sm">
            <thead className="bg-white text-xs font-black text-slate-500">
              <tr>
                {["الحساب", "المندوبون", "المصادر", "المشروع", "المدينة", "الشهر", "سبب التنبيه", "الحالة", "الإجراءات"].map((head) => (
              ...
              {result.alerts.map((alert) => (
                <tr key={alert.key} className="hover:bg-slate-50">
                  <td className="px-4 py-4">
                    <strong className="block text-slate-950">{alert.appUserId}</strong>
                    <span className="text-xs font-bold text-slate-500">{alert.appUsername}</span>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex flex-wrap gap-1">
                      {alert.drivers.map((driver) => (
                        <span key={driver} className="rounded-full bg-slate-100 px-2 py-1 text-xs font-black text-slate-700">{driver}</span>
                      ))}
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex flex-wrap gap-1">
                      {alert.sources.map((source) => (
                        <span key={source} className="rounded-full bg-blue-50 px-2 py-1 text-xs font-black text-blue-700">{source}</span>
                      ))}
                    </div>
                  </td>
                  ...
                  <td className="whitespace-nowrap px-4 py-4">
                    <span className={`rounded-full px-2 py-1 text-xs font-black ${statusBadge(alert.severity)}`}>
                      {alert.severity === "danger" ? "خطر" : "تحذير"}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex flex-wrap gap-2">
                      <Link href={`/settings/application-account-review${makeFilterLink(params, { applicationProjectId: alert.applicationProjectId, q: alert.appUserId })}`} className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs font-black text-amber-800">
                        مراجعة الحساب
                      </Link>
                      <Link href={`/drivers?q=${encodeURIComponent(alert.appUserId)}`} className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-black text-slate-700">
                        بحث في المناديب
                      </Link>
                    </div>
                  </td>
              ...
              {!result.alerts.length ? (
                <tr>
                  <td colSpan={9} className="px-4 py-10 text-center font-bold text-slate-500">
                    لا توجد تنبيهات حركة حسابات ضمن الفلاتر الحالية.
                  </td>
                </tr>
              ) : null}
            </tbody>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 bg-slate-50 px-4 py-3">
          <h2 className="text-lg font-black text-slate-950">سجل الحركات المحفوظة</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-right text-sm">
            <thead className="bg-white text-xs font-black text-slate-500">
              <tr>
                {["الحساب", "التطبيق", "من مندوب", "إلى مندوب", "نوع الحركة", "التاريخ", "الحالة", "ملاحظات"].map((head) => (
                  <th key={head} className="px-4 py-3">{head}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {result.movements.map((movement) => (
                <tr key={movement.id}>
                  <td className="whitespace-nowrap px-4 py-4 font-mono text-xs">{movement.accountId || "-"}</td>
                  <td className="whitespace-nowrap px-4 py-4 font-bold">{movement.appName || "-"}</td>
                  <td className="whitespace-nowrap px-4 py-4">{movement.fromDriverId || "-"}</td>
                  <td className="whitespace-nowrap px-4 py-4">{movement.toDriverId || "-"}</td>
                  <td className="whitespace-nowrap px-4 py-4">{movement.movementType}</td>
                  <td className="whitespace-nowrap px-4 py-4">{movement.movementDate.toISOString().slice(0, 10)}</td>
                  <td className="whitespace-nowrap px-4 py-4">{String(movement.status)}</td>
                  <td className="px-4 py-4">{movement.notes || "-"}</td>
                </tr>
              ))}
