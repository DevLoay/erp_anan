...
export async function GET(request: Request) {
  ...
  try {
    const data = await prisma.applicationInvoiceSetting.findMany({
      where: {
    ...
    return NextResponse.json({ data, meta: { count: data.length, resource: "invoice-settings" } });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  const payload: Prisma.ApplicationInvoiceSettingUncheckedCreateInput = {
      name,
      applicationId,
      applicationProjectId,
      invoiceType: nullableString(body.invoiceType),
      requiredColumns: jsonValue(body.requiredColumns),
      optionalColumns: jsonValue(body.optionalColumns),
      columnMapping: jsonValue(body.columnMapping),
      calculationRules: jsonValue(body.calculationRules),
      deductionRules: jsonValue(body.deductionRules),
      bonusRules: jsonValue(body.bonusRules),
      status: normalizeStatus(body.status),
  };

  const data = await prisma.applicationInvoiceSetting.create({ data: payload });
